subset_columns <- function(data,targetvariable,cols_in_rf){  #target variable is inputted as column vector
  ...
  for (i in 1:cols_in_rf){
    ...
    newdata[[colnames(data[random_col_num])]] <- data[,c(colnames(data[random_col_num]))] # this grabs the random column vector and assigns it
    # to the new subsetted dataset
