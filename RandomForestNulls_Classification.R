...
RF_with_Nulls <- function(data,data_to_score,targetvariable,percent_of_rows,cols_in_rf,iterations,complexity_parameter,minbucket,print_every_ithtree){
  ...
  data_to_score[["prediction_overall_geometric"]] <- 1

  for (i in 1:iterations){
    data_to_score[["prediction_overall"]]<-data_to_score[["prediction_overall"]] + data_to_score[[paste("prediction_", i, sep = "")]]
    data_to_score[["prediction_overall_geometric"]]<-data_to_score[["prediction_overall_geometric"]] * (1 - data_to_score[[paste("prediction_", i, sep = "")]])
  ...
  data_to_score[["prediction_overall_geometric"]] <- nthroot(data_to_score[["prediction_overall_geometric"]],iterations)
